from gokart_bot.formatting import format_laps, format_leaderboard, format_leaderboard_avg, format_myrecords, format_session, format_user_records
...
def session() -> SessionRecord:
    return SessionRecord(
        ...
        heat="Heat 12",
        karts=[
            KartResult(kart_no=12, position=1, best_lap=19.65, avg_lap=20.245, laps=[20.84, 19.65], claimed_by_name="driver"),
...
def test_session_display_uses_mentions_for_claimed_users() -> None:
    record = session()
    record.find_kart(12).claimed_by_user_id = 99

    output = format_session(record)

    assert "已認領：<@99>" in output
    assert "已認領：driver" not in output


def test_session_display_includes_average_lap() -> None:
    output = format_session(session())

    assert "平均 20.25s" in output
...
def test_leaderboard_avg_uses_each_claimed_driver_best_avg_once() -> None:
    first = session()
    first.find_kart(12).claimed_by_user_id = 99
    first.find_kart(12).avg_lap = 20.25
    first.karts.append(KartResult(kart_no=5, position=3, best_lap=18.5, avg_lap=19.0, claimed_by_name="unclaimed"))
    second = SessionRecord(
        id="2",
        channel_id=10,
        source_message_id=21,
        ...
        date="2026/5/17",
        printed_time="05:00:00",
        karts=[KartResult(kart_no=12, position=1, best_lap=20.0, avg_lap=21.0, claimed_by_user_id=99, claimed_by_name="driver")],
    )

    output = format_leaderboard_avg([first, second])

    assert "1. 20.25s｜<@99>｜2026/5/16 04:18:42" in output
    assert "21.00s" not in output
    assert "19.00s" not in output
    assert "最佳平均" in output


def test_profile_display_includes_mvp_stats() -> None:
    record = session()
    record.find_kart(12).claimed_by_user_id = 99
    output = format_user_records(99, [record], "<@99>")

    assert "<@99>的卡丁車紀錄" in output
